    ...
    <title>Data Obat || Apotek Siaga</title>
                                    ...
                                    <table class="table table-striped table-bordered first" id="datatable">
                                        ...
                                        <tbody>                                            
                                                <%    
                                                    ...
                                                    request.setAttribute("list", data); int x=1;
                                                %>
                                                <c:forEach items="${list}" var="u">
                                                <tr>
                                                    <td><% out.print(x++); %></td>
                                                    <td>${u.getNama_obat()}</td>
                                                    <td>${u.getId_jenis()}</td>
                                                    <td>${u.getKeterangan()}</td>
                                                    <td>${u.getDosis()}</td>
                                                    <td>${u.getStok()}</td>
                                                    <td>${u.getHarga()}</td>
                                                    <td align="center"><a href="form-update-obat.jsp?kd_obat=${u.getKd_obat()}">
                                                       <button class="btn btn-dark"><span class="fa fa-edit"></span> Update</button></a>
                                                        <a href="delete-obat.jsp?kd_obat=${u.getKd_obat()}" class="delete">
                                                            <button class="btn btn-danger"><span class="fa fa-trash"></span> Delete</button></a></td>
                                                </tr>
                                                </c:forEach>
                                        </tbody>
                                        <div style="margin-bottom: 20px;">                                             
                                            <a href="form-add-obat.jsp"><button class="btn btn-primary">
                                                    <span class="fa fa-plus"></span> Add Obat</button></a>
                                        </div>
                                    </table>
                                </div>
                            </div>
        ...
        $(document).ready(function(){
            $(".delete").on("click",function(){
                ...
                Swal.fire({
                    ...
                    text: 'Beberapa data tidak bisa dihapus karena tersimpan di tabel Penjualan!',
                    type: 'warning',
